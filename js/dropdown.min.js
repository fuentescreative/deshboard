...
+(function (d) {
			...
			d('[data-toggle="dropdown"]').each(function () {
				...
				!c.hasClass('open') ||
					(a &&
						...
						d.contains(c[0], a.target)) ||
					(c.trigger((a = d.Event('hide.bs.dropdown', e))),
					a.isDefaultPrevented() ||
						(b.attr('aria-expanded', 'false'),
						c.removeClass('open').trigger(d.Event('hidden.bs.dropdown', e))));
			}));
	}
	var e = function (a) {
		...
		if (
			...
			if (!b.is('.disabled, :disabled')) {
				...
				if ((!e && 27 != a.which) || (e && 27 == a.which))
					return (
						27 == a.which &&
							c.find('[data-toggle="dropdown"]').trigger('focus'),
						b.trigger('click')
					);
	...
	d(document)
		.on('click.bs.dropdown.data-api', g)
